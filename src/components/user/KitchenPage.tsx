              ...
              <div key={day}>
                ...
                <div className="space-y-1.5 pl-9">
                  {MEAL_ORDER.filter((mt) =>
                    meals.some((m) => m.dayNumber === day && m.mealTime === mt),
                  ).map((mt) => {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border flex-shrink-0">
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              {plan.durationDays <= 7 ? "per week" : "per month"}
            </p>
          </div>
...
function SubscribeModal({
  ...
  const isValid =
    ...
    (deliveryOption === "PICKUP" || address.trim().length > 0);
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {plan.durationDays <= 7 ? "week" : "month"}
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["DELIVERY", "PICKUP"] as DeliveryOption[]).map((opt) => (
                <button
                  ...
                  disabled={
                    (opt === "DELIVERY" && !plan.isDeliveryAvailable) ||
                    (opt === "PICKUP" && !plan.isPickupAvailable)
                  }
          ...
          {deliveryOption === "DELIVERY" && (
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. 14 Mwangi Road, Westlands"
                className="rounded-xl border-border"
              />
            </div>
          )}
          <div className="bg-muted/40 rounded-xl p-3 space-y-1.5 text-xs">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {plan.durationDays * plan.mealsPerDay}
              </span>
            </div>
...
function MenuItemCard({
  ...
  const { addToplan, isPlanned, removeFromPlan, plannedMeals, loading } =
    usePlan();
      ...
      <div className="relative w-full aspect-[4/3] bg-muted overflow-hidden flex-shrink-0">
        ...
        {!item.isAvailable && (
          <div className="absolute inset-0 bg-background/70 flex items-center justify-center">
            <span className="text-xs font-black text-muted-foreground border border-border px-3 py-1 rounded-full bg-background">
              Unavailable
            </span>
          </div>
        )}
      ...
      <div className="p-3 flex flex-col gap-2 flex-1">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {item.mealTimes.map((mt) => (
            <span
              key={mt}
              className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-secondary/20 text-secondary-foreground"
            >
              {mt.charAt(0) + mt.slice(1).toLowerCase()}
            </span>
          ))}
...
export default function KitchenPage({ businessId }: { businessId?: string }) {
  ...
  const fetchMenu = useCallback(async () => {
    ...
    setLoadingMenu(true);
    try {
      const res = await fetch(`/api/business/${businessId}/menu?limit=100`, {
        cache: "no-store",
      });
      const json = await res.json();
      const raw = json?.items ?? json?.data ?? [];
      setMenuItems(
        raw.map((item: MenuItem & { price: number | string }) => ({
          ...item,
  ...
  const fetchMealPlans = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/business/${businessId}/meal-plans?limit=50`,
        { cache: "no-store" },
      );
      const json = await res.json();
      const raw = json?.data ?? [];
      setMealPlans(
        raw.map((p: MealPlan & { price: number | string }) => ({
          ...p,
          price: Number(p.price),
          subscriberCount:
            p.subscriberCount ??
            (p as unknown as { currentSubscribers?: number })
              .currentSubscribers ??
            0,
  ...
  useEffect(() => {
    fetchBusiness();
    fetchMenu();
  }, [fetchBusiness, fetchMenu]);
  useEffect(() => {
    if (activeTab === "plans") fetchMealPlans();
  }, [activeTab, fetchMealPlans]);
  ...
  const paginatedItems = filteredItems.slice(
    (page - 1) * PAGE_SIZE_MENU,
    page * PAGE_SIZE_MENU,
  );
  const paginatedPlans = mealPlans.slice(
    (page - 1) * PAGE_SIZE_PLANS,
    page * PAGE_SIZE_PLANS,
  );
          ...
          {business?.premiseImageUrl ? (
            ...
            <>
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, ${accentColor}f0 0%, ${accentColor}b0 35%, ${accentColor}60 65%, ${accentColor}20 100%)`,
                }}
              />
              <div
                className="absolute inset-0"
                style={{
                  background:
                    "linear-gradient(225deg, rgba(255,220,80,0.2) 0%, transparent 55%)",
                }}
              />
              {/* Dot grid texture */}
              <div
              ...
              <div className="flex items-end justify-between gap-4 flex-wrap">
                ...
                >
                  ...
                  {business?.services.map((s) => (
                    <div
                      key={s}
                      className="flex items-center gap-1.5 bg-white/10 backdrop-blur-md border border-white/20 rounded-xl px-3 py-2 text-white"
                    >
                      {s === "DELIVERY" && (
                      ...
                      {s === "DINE_IN" && (
                        <UtensilsCrossed className="w-3.5 h-3.5 text-white/70" />
                      )}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-5">
            ...
            <div className="flex flex-col sm:flex-row gap-0">
              ...
              <div className="flex items-center gap-4 p-5 sm:border-r border-border sm:min-w-[260px]">
                <div className="relative flex-shrink-0">
                  <div
                    className="w-16 h-16 rounded-2xl border-2 border-border shadow-sm flex items-center justify-center text-3xl overflow-hidden"
                ...
                {business.foodSpecialty.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {business.foodSpecialty.map((s) => (
                      <span
                        key={s}
                        className="text-[11px] font-semibold px-2.5 py-1 rounded-lg border border-border bg-muted/50 text-foreground"
                      >
                        {s}
                      </span>
                    ))}
              ...
              {business.availability.length > 0 && (
                <div className="sm:border-l border-t sm:border-t-0 border-border p-5 flex flex-col justify-center gap-1.5 sm:min-w-[140px]">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"].map(
                      (d, i) => {
                        ...
                        const isOpen = business.availability.includes(
                          dayMap[d] as DayOfWeek,
                        );
                        return (
                          <span
                            key={i}
                            className={`text-[9px] font-black w-6 h-6 rounded-md flex items-center justify-center ${
                              isOpen
                  ...
                  {business.chef.profile?.phone && (
                    <a
                      href={`tel:${business.chef.profile.phone}`}
        ...
        {mounted && planCount > 0 && (
          ...
          >
            ...
            {planCount} meal{planCount !== 1 ? "s" : ""} added to schedule
          </motion.a>
        )}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-background/95 backdrop-blur-sm rounded-2xl border border-border px-4 py-3 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() =>
                activeTab === "menu" ? fetchMenu() : fetchMealPlans()
              }
