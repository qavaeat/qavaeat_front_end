...
const INITIAL: FormState = {
  ...
  cuisineType: [], mealTypes: [], price: "",
  currency: "KES", durationDays: "", mealsPerDay: "",
  maxSubscribers: "", isDeliveryAvailable: true,
  isPickupAvailable: true, availableDays: [], tags: [],
};
...
function validate(form: FormState): FieldErrors {
  ...
  if (form.cuisineType.length === 0) e.cuisineType = "Add at least one cuisine type.";
  if (form.mealTypes.length === 0) e.mealTypes = "Select at least one meal type.";
  ...
  if (!form.mealsPerDay || Number(form.mealsPerDay) < 1) e.mealsPerDay = "At least 1 meal per day.";
...
export function AddMealPlanForm({ onClose, onSave }: Props) {
  ...
  const toggleMealType = (mt: MealTime) =>
    set("mealTypes", form.mealTypes.includes(mt) ? form.mealTypes.filter((m) => m !== mt) : [...form.mealTypes, mt]);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border sticky top-0 bg-background z-10">
          <h2 className="text-base font-black text-foreground">Add Meal Plan</h2>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-1.5">
              ...
              {errors.durationDays && <p className="text-[10px] text-destructive">{errors.durationDays}</p>}
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-foreground">Meals/day *</label>
              <Input type="number" value={form.mealsPerDay} onChange={(e) => set("mealsPerDay", e.target.value)}
                placeholder="e.g. 3"
                className={`rounded-xl ${errors.mealsPerDay ? "border-destructive" : "border-border"}`} />
              {errors.mealsPerDay && <p className="text-[10px] text-destructive">{errors.mealsPerDay}</p>}
            </div>
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            {errors.mealTypes && <p className="text-[10px] text-destructive">{errors.mealTypes}</p>}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            {errors.availableDays && <p className="text-[10px] text-destructive">{errors.availableDays}</p>}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              ...
              <Button type="button" onClick={addCuisine} variant="outline" className="rounded-xl border-border px-3">
                <Plus className="w-4 h-4" />
              </Button>
            </div>
            {errors.cuisineType && <p className="text-[10px] text-destructive">{errors.cuisineType}</p>}
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <Input value={tagInput} onChange={(e) => setTagInput(e.target.value)}
                ...
                className="rounded-xl border-border flex-1 text-sm" />
              <Button type="button" onClick={addTag} variant="outline" className="rounded-xl border-border px-3">
                <Plus className="w-4 h-4" />
              </Button>
            </div>
            {form.tags.length > 0 && (
