...
function validate(form: MenuItem): FieldErrors {
  ...
  if (form.prepTimeMin !== null && Number(form.prepTimeMin) < 1)
    e.prepTimeMin = "Prep time must be at least 1 minute.";
  return e;
}

export function UpdateMenuItemForm({ item, onClose, onSave, onDelete }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = { ...form, imageUrl, price: Number(form.price) };

      const res = await fetch(`/api/chefs/menu/${item.id}`, {
        method: "PATCH",
      ...
      onSave(payload);  // updates the list in TabMenus instantly
      onClose();
    } catch (err: unknown) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      onDelete?.(item.id); // removes item from list in TabMenus instantly
      onClose();
    } catch (err: unknown) {
  ...
  return (
    <>
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                />
                {errors.prepTimeMin && (
                  <p className="text-[10px] text-destructive">{errors.prepTimeMin}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              {errors.mealTimes && (
                <p className="text-[10px] text-destructive">{errors.mealTimes}</p>
              )}
            </div>
      ...
      {/* Delete confirmation dialog — renders above the edit modal */}
      <ConfirmDeleteDialog
