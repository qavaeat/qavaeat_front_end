...
function validate(form: FormState): FieldErrors {
  ...
  if (form.mealTimes.length === 0)
    e.mealTimes = "Select at least one meal time.";
  if (form.prepTimeMin && (isNaN(Number(form.prepTimeMin)) || Number(form.prepTimeMin) < 1))
    e.prepTimeMin = "Prep time must be at least 1 minute.";
  return e;
}

export function AddMenuItemForm({ onClose, onSave }: Props) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border sticky top-0 bg-background z-10">
          <h2 className="text-base font-black text-foreground">Add Menu Item</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. Grilled Chicken Nuggets"
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-1">
                <button
                  type="button"
                  onClick={() => set("quantity", String(Math.max(0, Number(form.quantity) - 1)))}
          ...
          <div className="flex items-center justify-between py-3 px-4 rounded-xl bg-muted/40 border border-border">
            ...
            <button
              ...
              className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${
                form.isAvailable ? "bg-[#007606]" : "bg-muted-foreground/30"
              }`}
            >
              <span
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border sticky bottom-0 bg-background">
          ...
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            <Button
              onClick={handleSave}
              disabled={saving}
              className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full px-8 font-bold tracking-wide"
            >
              {saving ? (
