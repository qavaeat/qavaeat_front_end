...
function validate(form: FormState): FieldErrors {
  ...
  if (
    form.prepTimeMin &&
    (isNaN(Number(form.prepTimeMin)) || Number(form.prepTimeMin) < 1)
  )
...
export function AddMenuItemForm({ onClose, onSave }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.post("/chefs/menu", payload);

      toast.success("Menu item added!");
      onSave(payload as Omit<MenuItem, "id">); // ← add this line
      onClose();
    } catch (err: unknown) {
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  onClick={() =>
                    set(
                      "quantity",
                      String(Math.max(0, Number(form.quantity) - 1)),
                    )
                  }
                ...
                <button
                  type="button"
                  onClick={() =>
                    set("quantity", String(Number(form.quantity) + 1))
                  }
                  className="w-8 h-8 rounded-lg border border-border flex items-center justify-center hover:bg-muted transition-colors"
                >
